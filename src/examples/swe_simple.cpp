...
int main( int argc, char** argv ) {
...
#ifdef USE_SCALASCA
  EPIK_FUNC_START();
  EPIK_USER_REG(r_name, "main_region");
  ...
  float l_checkPoints[l_numberOfCheckPoints+1];

#ifdef USE_SCALASCA
  EPIK_USER_REG(r_name_comp_checkpoints, "main_region_init_checkpoints");
  EPIK_USER_START(r_name_comp_checkpoints);  
#endif

#ifdef VECTORIZE
	// NOTE: Not (significantly) faster than unvectorized, probably due to auto-vectorisation of intel-compiler (-O3)
	float l_factor = l_endSimulation/l_numberOfCheckPoints;
	l_checkPoints[:] = __sec_implicit_index(0)*l_factor;
#else
...
#ifdef USE_SCALASCA
  EPIK_USER_REG(r_name_loop, "main_loop");
  EPIK_USER_START(r_name_loop);  
#endif
